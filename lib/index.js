...
export default function readme(config = {}) {
    ...
    const pkgBasename = !isDefault('name') ? getPackageBasename(pkg.name) : '';

    const pkgRequireStatement = !isDefault('name') ? `const ${camelCase(pkgBasename)} = require('${pkg.name}');` : '';

    const pkgImportStatement = !isDefault('name') ? `import ${camelCase(pkgBasename)} from '${pkg.name}';` : '';

    const pkgUsageStatement = pkg?.type === 'module' ? pkgImportStatement : pkgRequireStatement;
    ...
    additionalSections.forEach((section) => {
        ...
        /**
         * @type {import('./ReadmeSections.d.ts').ReadmeSection}
         */
